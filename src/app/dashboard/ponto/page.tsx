...
export default function PontoPage() {
    ...
    const isAdmin = React.useMemo(() => {
        return currentMember?.role && ['Diretor', 'Presidente', 'Assessor'].includes(currentMember.role);
    }, [currentMember]);
    ...
    const calculateMemberStatus = (member: Member): MemberStatus => {
        ...
        const lastWeekStart = startOfWeek(boxDate(subWeeks(today, 1)), { weekStartsOn: 1 });
        const lastWeekEnd = endOfWeek(boxDate(subWeeks(today, 1)), { weekStartsOn: 1 });
        ...
        for (const record of records) {
            ...
            if (record.type === 'Entrada') {
                ...
                const duration = (recordDate.getTime() - lastEntrada.getTime()) / (1000 * 60);
        ...
        if (isWorking && lastRecord?.timestamp?.toDate) {
            ...
            const now = new Date();
            const currentDuration = (now.getTime() - entryTime.getTime()) / (1000 * 60);
    ...
    return (
        <PageContainer>
            <div className="flex flex-col gap-6">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Gerenciamento de Ponto</h1>
